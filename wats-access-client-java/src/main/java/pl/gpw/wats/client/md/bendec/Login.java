...
public class Login implements ByteSerializable, Message {
    ...
    public Login(byte[] bytes, int offset) {
        ...
        this.connectionId = BendecUtils.uInt16FromByteArray(bytes, offset + 39);
        this.token = BendecUtils.stringFromByteArray(bytes, offset + 41, 8);
        this.header.setLength(this.byteLength);
        this.header.setMsgType(MsgType.LOGIN);
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(header, connectionId, token);
    }
