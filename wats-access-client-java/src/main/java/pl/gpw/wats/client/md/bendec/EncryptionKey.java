...
public class EncryptionKey implements ByteSerializable, Message {
    ...
    public static final int byteLength = 75;
    ...
    public EncryptionKey(Header header, long id, String secretKey) {
        ...
        this.header.setMsgType(MsgType.ENCRYPTIONKEY);
    }

    public EncryptionKey(byte[] bytes, int offset) {
        this.header = new Header(bytes, offset);
        this.id = BendecUtils.uInt32FromByteArray(bytes, offset + 39);
        this.secretKey = BendecUtils.stringFromByteArray(bytes, offset + 43, 32);
        this.header.setLength(this.byteLength);
        this.header.setMsgType(MsgType.ENCRYPTIONKEY);
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(header, id, secretKey);
    }
