...
public class TopPriceLevelUpdate implements ByteSerializable, Message {
    ...
    public TopPriceLevelUpdate(Header header, long instrumentId, long price, BigInteger quantity, OrderSide side, int orderCount) {
        ...
        this.header.setMsgType(MsgType.TOPPRICELEVELUPDATE);
    }

    public TopPriceLevelUpdate(byte[] bytes, int offset) {
        this.header = new Header(bytes, offset);
        this.instrumentId = BendecUtils.uInt32FromByteArray(bytes, offset + 39);
        this.price = BendecUtils.int64FromByteArray(bytes, offset + 43);
        this.quantity = BendecUtils.uInt64FromByteArray(bytes, offset + 51);
        this.side = OrderSide.getOrderSide(bytes, offset + 59);
        this.orderCount = BendecUtils.uInt16FromByteArray(bytes, offset + 60);
        this.header.setLength(this.byteLength);
        this.header.setMsgType(MsgType.TOPPRICELEVELUPDATE);
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(header, instrumentId, price, quantity, side, orderCount);
    }
