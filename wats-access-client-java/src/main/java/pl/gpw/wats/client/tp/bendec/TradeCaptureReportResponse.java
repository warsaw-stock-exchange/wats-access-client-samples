...
public class TradeCaptureReportResponse implements ByteSerializable, Message {
    ...
    public TradeCaptureReportResponse(byte[] bytes, int offset) {
        ...
        this.tradeId = BendecUtils.uInt32FromByteArray(bytes, offset + 28);
        this.tradeReportId = BendecUtils.stringFromByteArray(bytes, offset + 32, 21);
        this.secondaryTradeReportId = BendecUtils.uInt64FromByteArray(bytes, offset + 53);
        this.status = TcrStatus.getTcrStatus(bytes, offset + 61);
        this.reason = TcrRejectionReason.getTcrRejectionReason(bytes, offset + 62);
    }
