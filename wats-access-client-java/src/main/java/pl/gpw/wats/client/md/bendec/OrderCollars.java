...
public class OrderCollars implements ByteSerializable, Message {
    ...
    public OrderCollars(Header header, long instrumentId, CollarType collarType, long price, long lowerBid, long upperBid, long lowerAsk, long upperAsk) {
        ...
        this.header.setMsgType(MsgType.ORDERCOLLARS);
    }

    public OrderCollars(byte[] bytes, int offset) {
        this.header = new Header(bytes, offset);
        this.instrumentId = BendecUtils.uInt32FromByteArray(bytes, offset + 39);
        this.collarType = CollarType.getCollarType(bytes, offset + 43);
        this.price = BendecUtils.int64FromByteArray(bytes, offset + 44);
        this.lowerBid = BendecUtils.int64FromByteArray(bytes, offset + 52);
        this.upperBid = BendecUtils.int64FromByteArray(bytes, offset + 60);
        this.lowerAsk = BendecUtils.int64FromByteArray(bytes, offset + 68);
        this.upperAsk = BendecUtils.int64FromByteArray(bytes, offset + 76);
        this.header.setLength(this.byteLength);
        this.header.setMsgType(MsgType.ORDERCOLLARS);
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(header, instrumentId, collarType, price, lowerBid, upperBid, lowerAsk, upperAsk);
    }
