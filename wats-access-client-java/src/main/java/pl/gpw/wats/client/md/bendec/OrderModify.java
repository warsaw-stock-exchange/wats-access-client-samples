...
public class OrderModify implements ByteSerializable, Message {
    ...
    public OrderModify(byte[] bytes, int offset) {
        ...
        this.publicOrderId = BendecUtils.uInt64FromByteArray(bytes, offset + 43);
        this.price = BendecUtils.int64FromByteArray(bytes, offset + 51);
        this.quantity = BendecUtils.uInt64FromByteArray(bytes, offset + 59);
        this.priorityFlag = PriorityFlag.getPriorityFlag(bytes, offset + 67);
        this.header.setLength(this.byteLength);
        this.header.setMsgType(MsgType.ORDERMODIFY);
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(header, instrumentId, publicOrderId, price, quantity, priorityFlag);
    }
