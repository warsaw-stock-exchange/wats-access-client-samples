...
public class RealTimeIndex implements ByteSerializable, Message {
    ...
    public static final int byteLength = 126;
    ...
    public RealTimeIndex(Header header, long instrumentId, IndexLevelCode indexLevelCode, int numOfActiveInstruments, long indIndexOpeningPortfolio, long indexValue, long pctChangeIndexValPrevSession, long tradingValue, long sessionLow, long sessionHigh, long indexValueBid, long indexValueAsk, long midSpreadIndex, long differenceCentralSpread) {
        ...
        this.header.setMsgType(MsgType.REALTIMEINDEX);
    }

    public RealTimeIndex(byte[] bytes, int offset) {
        this.header = new Header(bytes, offset);
        this.instrumentId = BendecUtils.uInt32FromByteArray(bytes, offset + 39);
        this.indexLevelCode = IndexLevelCode.getIndexLevelCode(bytes, offset + 43);
        this.numOfActiveInstruments = BendecUtils.uInt16FromByteArray(bytes, offset + 44);
        this.indIndexOpeningPortfolio = BendecUtils.int64FromByteArray(bytes, offset + 46);
        this.indexValue = BendecUtils.int64FromByteArray(bytes, offset + 54);
        this.pctChangeIndexValPrevSession = BendecUtils.int64FromByteArray(bytes, offset + 62);
        this.tradingValue = BendecUtils.int64FromByteArray(bytes, offset + 70);
        this.sessionLow = BendecUtils.int64FromByteArray(bytes, offset + 78);
        this.sessionHigh = BendecUtils.int64FromByteArray(bytes, offset + 86);
        this.indexValueBid = BendecUtils.int64FromByteArray(bytes, offset + 94);
        this.indexValueAsk = BendecUtils.int64FromByteArray(bytes, offset + 102);
        this.midSpreadIndex = BendecUtils.int64FromByteArray(bytes, offset + 110);
        this.differenceCentralSpread = BendecUtils.int64FromByteArray(bytes, offset + 118);
        this.header.setLength(this.byteLength);
        this.header.setMsgType(MsgType.REALTIMEINDEX);
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(header, instrumentId, indexLevelCode, numOfActiveInstruments, indIndexOpeningPortfolio, indexValue, pctChangeIndexValPrevSession, tradingValue, sessionLow, sessionHigh, indexValueBid, indexValueAsk, midSpreadIndex, differenceCentralSpread);
    }
