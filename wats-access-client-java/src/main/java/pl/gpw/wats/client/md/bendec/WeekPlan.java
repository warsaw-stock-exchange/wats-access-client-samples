...
public class WeekPlan implements ByteSerializable, Message {
    ...
    public static final int byteLength = 50;
    ...
    public WeekPlan(Header header, long calendarId, boolean[] weekdays) {
        ...
        this.header.setMsgType(MsgType.WEEKPLAN);
    }
    ...
    public WeekPlan(byte[] bytes, int offset) {
        ...
        for(int i = 0; i < weekdays.length; i++) {
            this.weekdays[i] = BendecUtils.booleanFromByteArray(bytes, offset + 43 + i * 1);
        }
        this.header.setLength(this.byteLength);
        this.header.setMsgType(MsgType.WEEKPLAN);
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(header, calendarId, weekdays);
    }
