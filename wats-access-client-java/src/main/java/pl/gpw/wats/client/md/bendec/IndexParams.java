 ...
 * <p>IndexUnderlying > PublicProductIdentification[] (PublicProductIdentification[]) indexUnderlying - List of underlying instruments for index. | size 93</p>
 * <p>u16 > int publicationOrder - Index publication presentation order. | size 2</p>
 * <p>Currency currency - Currency (e.g. USD). | size 2</p>
 * <p>Date > long (u32) dateValidity - Date of validity of index. | size 4</p>
 * */
...
public class IndexParams implements ByteSerializable, Message {
    ...
    public IndexParams(Header header, long instrumentId, long indexBaseValue, long indexBaseCapitalisation, long indexBaseDate, long correctionCoefficient, int numOfComponents, IndexPublicationSchedule indexPublicationSchedule, IndexType indexType, int daysSinceLastPublication, int numberOfDividends, PublicProductIdentification[] indexUnderlying, int publicationOrder, Currency currency, long dateValidity) {
        ...
        this.header.setMsgType(MsgType.INDEXPARAMS);
    }

    public IndexParams(byte[] bytes, int offset) {
        this.header = new Header(bytes, offset);
        this.instrumentId = BendecUtils.uInt32FromByteArray(bytes, offset + 39);
        this.indexBaseValue = BendecUtils.int64FromByteArray(bytes, offset + 43);
        this.indexBaseCapitalisation = BendecUtils.int64FromByteArray(bytes, offset + 51);
        this.indexBaseDate = BendecUtils.uInt32FromByteArray(bytes, offset + 59);
        this.correctionCoefficient = BendecUtils.int64FromByteArray(bytes, offset + 63);
        this.numOfComponents = BendecUtils.uInt16FromByteArray(bytes, offset + 71);
        this.indexPublicationSchedule = IndexPublicationSchedule.getIndexPublicationSchedule(bytes, offset + 73);
        this.indexType = IndexType.getIndexType(bytes, offset + 74);
        this.daysSinceLastPublication = BendecUtils.uInt8FromByteArray(bytes, offset + 75);
        this.numberOfDividends = BendecUtils.uInt16FromByteArray(bytes, offset + 76);
        this.indexUnderlying = new PublicProductIdentification[3];
        for(int i = 0; i < indexUnderlying.length; i++) {
            this.indexUnderlying[i] = new PublicProductIdentification(bytes, offset + 78 + i * 31);
        }
        this.publicationOrder = BendecUtils.uInt16FromByteArray(bytes, offset + 171);
        this.currency = Currency.getCurrency(bytes, offset + 173);
        this.dateValidity = BendecUtils.uInt32FromByteArray(bytes, offset + 175);
        this.header.setLength(this.byteLength);
        this.header.setMsgType(MsgType.INDEXPARAMS);
    }
     ...
     */
    public PublicProductIdentification[] getIndexUnderlying() {
        return this.indexUnderlying;
    };
    /**
    ...
    /**
     * @return Date of validity of index.
     */
    ...
    /**
     * @param indexUnderlying List of underlying instruments for index.
     */
    public void setIndexUnderlying(PublicProductIdentification[] indexUnderlying) {
        this.indexUnderlying = indexUnderlying;
    };
    /**
    ...
    /**
     * @param dateValidity Date of validity of index.
     */
    ...
    public byte[] toBytes() {
        ...
        for(int i = 0; i < indexUnderlying.length; i++) {
            indexUnderlying[i].toBytes(buffer);
        }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(header, instrumentId, indexBaseValue, indexBaseCapitalisation, indexBaseDate, correctionCoefficient, numOfComponents, indexPublicationSchedule, indexType, daysSinceLastPublication, numberOfDividends, indexUnderlying, publicationOrder, currency, dateValidity);
    }
