...
class delayed_market_data_exception: exception {
private:
	std::string m_error{};
public:
	delayed_market_data_exception(std::string_view error): m_error{error} {}
...
private:
    ...
    template <typename T>
    void send(const T& t) {
        write(socket_replay_, buffer((char*) &t, sizeof(T)));
    ...
    void logout();
public:
    DelayedMarketData(io_context& io_context);
    ~DelayedMarketData();
    ...
    void start(const std::string streamHost, const uint16_t streamPort,
