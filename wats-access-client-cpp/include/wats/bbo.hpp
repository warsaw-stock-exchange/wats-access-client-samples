...
class best_bid_offer_exception: exception {
private:
	std::string m_error{};
public:
	best_bid_offer_exception(std::string_view error): m_error{error} {}
...
private:
    ...
    void logout();
public:
    BestBidOffer(io_context& io_context);
    ~BestBidOffer();
    ...
    /// @brief Initialize Best Bid Offer connection.
    /// @param config Node of YAML configuration file.
    void start(const YAML::Node &config);

    /// @brief Initialize Best Bid Offer connection.
    /// @param snapshotHost IP address (numeric or symbolic) of the Best Bid Offer snapshot link.
    /// @param snapshotPort Port number of the Best Bid Offer snapshot link.
    /// @param streamHost IP address (numeric or symbolic) of the Best Bid Offer stream (UDP) link.
    /// @param streamPort Port number of the Best Bid Offer stream (UDP) link.
    /// @param replayHost IP address (numeric or symbolic) of the Best Bid Offer replay link.
    /// @param replayPort Port number of the Best Bid Offer replay link.
    ...
    /// @brief Finalize Best Bid Offer connection.
    void stop();
};
